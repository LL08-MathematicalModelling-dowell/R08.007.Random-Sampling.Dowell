...
def FieldRP(length, width, selection, N):
...
def ExcelRP(length, width, selection):
  N=length
  area=length*width                         #calculate area of rectangle
  area_of_circle=area/N                     #calculate area of circle
  radius=sqrt(area_of_circle/pi)            #calculate the radius of circle
...
def random_allocation_function(data):
  ...
  if (selected_type=='fieldrp'):
    ...
    N = data['N']
    list1=FieldRP(length, width, selection, N)
    image = plotting_points(list1)
    random_allocation_data['listOfPoints']=list1
    random_allocation_data['image']=image
  if (selected_type=='excelrp'):
    length = data['columns']
    width = data['rows']
    selection = data['selection']
    list1=ExcelRP(length, width, selection)
    image = plotting_points(list1)
    random_allocation_data['listOfPoints']=list1
    random_allocation_data['image']=image
  
  dowell_connection(random_allocation_data)
  print(random_allocation_data)
